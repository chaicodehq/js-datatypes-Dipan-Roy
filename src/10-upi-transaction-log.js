...
export function analyzeUPITransactions(transactions) {
  ...
  // frequentContact: "to" appearing most often; tie → first appearance
  const contactCount = valid.reduce((acc, t) => {
    acc[t.to] = (acc[t.to] || 0) + 1;
    return acc;
  }, {});

  const frequentContact = valid.reduce((best, t) => {
    return contactCount[t.to] > contactCount[best] ? t.to : best;
  }, valid[0].to);
